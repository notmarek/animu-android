...
class FilePickerActivity : AppCompatActivity(), AbstractFilePickerFragment.OnFilePickedListener {
    ...
    private fun initFilePicker() {
        ...
        (fragment as MPVFilePickerFragment).goToDir(
            AnimuFile(
                path
            )
        )
    ...
    override fun onFilePicked(file: AnimuFile) = finishWithResult(RESULT_OK, file.path)

    override fun onDirPicked(dir: AnimuFile) = finishWithResult(RESULT_OK, dir.path)
